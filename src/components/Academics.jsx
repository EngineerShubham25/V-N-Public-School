  ...
  return (
    <section id="academics" className="py-16 bg-white border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Responsive Grid on Desktop / Touch Carousel on Mobile */}
        <div 
          onScroll={handleScroll}
          className="flex md:grid md:grid-cols-3 gap-4 md:gap-6 overflow-x-auto md:overflow-visible snap-x snap-mandatory no-scrollbar pb-2"
        >
          {grades.map((g, idx) => (
            <div 
              key={idx} 
              className={`min-w-[85vw] md:min-w-0 snap-center shrink-0 p-6 rounded-2xl border ${g.color} space-y-3 flex flex-col justify-between shadow-sm`}
            >
              <div>
          ...
          {grades.map((_, idx) => (
            <span
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                activeIndex === idx ? 'w-5 bg-blue-900' : 'w-2 bg-slate-300'
              }`}
            />
          ))}
        </div>
